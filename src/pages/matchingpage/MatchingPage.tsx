...
export default class MatchingPage extends Component<any, any> {
    ...
    configureChannel(channel: any) {

        channel.on("initial_matches", (response: any) => {
        ...
        channel.on("new_match", (response: any) => {
        ...
        channel.on("match_requested", (response: any) => {
        ...
        channel.on("remove_player", (response: any) => {
            console.log("Player left");
            this.removePlayer(response.id);
        });

        channel.on("request_response", (response: any) => {
    ...
    handleAccept = (response: any) => {
        this.setState({
            ...
            history.pushState(process.env.PUBLIC_URL + "/matchfound");
        });
    }

    handleReject = (response: any) => {
    ...
    removePlayer = (playerId: any) => {
        this.setState((prevState: any) => ({
            matches: prevState.matches.filter((match: any) => match.id !== playerId)
        }));
    }

    requestMatch = (player: any) => {
    ...
    respondToRequest = (playerId: any, requestResponse: any) => {
        ...
        return (
            <div className="main-content2">
                <div className="width-control2">
                    ...
                    <MatchTable matches={this.state.matches} requestMatch={this.requestMatch} />
